...
assert.match(introductionSource, /getTeachingSentenceHTML\(/);
assert.match(source, /function getTeachingSentenceHTML[\s\S]*?game-introduction-target/);
assert.match(source, /window\.Inflections\?\.getSentenceForms\?\.\(wordObj\)/);
assert.match(source, /window\.SentenceFormMatching\?\.createMatcher\(forms\)/);
assert.match(source, /window\.ExpressionPatterns\?\.getAnalysis\(wordObj\)/);
assert.match(source, /<mark class="game-introduction-target">\$1<\/mark>/);
